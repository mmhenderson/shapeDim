...
for cc=1:length(cope2use)
    ...
    if strcmp(subnum,'05') && strcmp(subinit,'CG')  
        % this subject has just one run, therefore we must do the stats differently.
        stats_path = [exp_path 'AnalyzeCategLoc/S' char(subnum) '/feats/S05_04_001.feat/stats'];
        ...
        stats_path = [exp_path 'AnalyzeCategLoc/S' char(subnum) '/feats/AllSessionsFE.gfeat/cope' num2str(cope2use(cc)) '.feat/stats/'];
        cd(stats_path)
    %     % load my t-stat map for all voxels
