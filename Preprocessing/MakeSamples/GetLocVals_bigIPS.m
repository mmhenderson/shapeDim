function [] = GetLocVals_bigIPS(sub2do, debug)

%Made this so that we can get the t-statisics from the silhouette localizer task
%Will be used to choose voxels for decoding analysis
%Could do this in the MakeSampleFile function, but this is faster than running that again.
...
for ss = sub2do
    ...
    fn2save = fullfile(out_path,sprintf('LocVals_bigIPS_%s.mat',substr));
    ...
    fprintf('t-threshold is: %.4f\n', tThresh)
    fprintf('min value is: %.4f\n', min(vmpVals(vox_above_thresh)))
    ...
    loc_vals = vmpVals(all_vox_concat);
    fprintf('min value overall is: %.4f\n', min(loc_vals))
    
    check_v1_vals = vmpVals(ROIs(1,1).voxel_inds);
    fprintf('min value V1 lh is: %.4f\n', min(check_v1_vals))
    
    check_v1_vals = vmpVals(ROIs(1,2).voxel_inds);
    fprintf('min value V1 rh is: %.4f\n', min(check_v1_vals))
    
    check_v1_inds = squeeze(find(ismember(all_vox_concat, ROIs(1,1).voxel_inds)))';
    fprintf('size of V1 lh is: %d\n', length(check_v1_inds))
    check_v1_vals = loc_vals(check_v1_inds);
    fprintf('min value V1 lh is: %.4f\n', min(check_v1_vals))
    
    check_v1_inds = squeeze(find(ismember(all_vox_concat, ROIs(1,2).voxel_inds)))';
    fprintf('size of V1 rh is: %d\n', length(check_v1_inds))
    check_v1_vals = loc_vals(check_v1_inds);
    fprintf('min value V1 rh is: %.4f\n', min(check_v1_vals))
