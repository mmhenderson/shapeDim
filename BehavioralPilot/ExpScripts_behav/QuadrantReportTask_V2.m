...
try
    ...
    mapping = input('Mapping? (1-4)  --> ','s'); 
    if isempty(mapping)  
        error('enter an integer between 1 and 4')
    end
    p.which_mapping = str2double(mapping);
    if mod(p.which_mapping,1) || p.which_mapping<1 || p.which_mapping>4
        error('enter an integer between 1 and 4')
    end
    ...
    if p.Training
        p.expName = sprintf('ShapeQuadTaskV2_mapping%d_TRAINING',p.which_mapping);
    else
        p.expName = sprintf('ShapeQuadTaskV2_mapping%d',p.which_mapping);
    end
...
%     
    ...
    % use the response mapping now to define which shape is which "quadrant"
    p.category = zeros(size(p.imlist));
    if p.which_mapping==1
        % p.category tells us which finger the subject should press on each
        % trial
        % these are going counter-clockwise from top right (in shape space)
        p.category(p.imcoords(:,1)>center & p.imcoords(:,2)>center) = 1;
        p.category(p.imcoords(:,1)<center & p.imcoords(:,2)>center) = 2;
        p.category(p.imcoords(:,1)<center & p.imcoords(:,2)<center) = 3;
        p.category(p.imcoords(:,1)>center & p.imcoords(:,2)<center) = 4;
        p.proto_order = [1,2,3,4];
    elseif p.which_mapping==2
        p.category(p.imcoords(:,1)>center & p.imcoords(:,2)>center) = 2;
        p.category(p.imcoords(:,1)<center & p.imcoords(:,2)>center) = 1;
        p.category(p.imcoords(:,1)<center & p.imcoords(:,2)<center) = 4;
        p.category(p.imcoords(:,1)>center & p.imcoords(:,2)<center) = 3;
        p.proto_order = [2,1,4,3];
    elseif p.which_mapping==3
        p.category(p.imcoords(:,1)>center & p.imcoords(:,2)>center) = 3;
        p.category(p.imcoords(:,1)<center & p.imcoords(:,2)>center) = 4;
        p.category(p.imcoords(:,1)<center & p.imcoords(:,2)<center) = 1;
        p.category(p.imcoords(:,1)>center & p.imcoords(:,2)<center) = 2;
        p.proto_order = [3,4,1,2];
    elseif p.which_mapping==4
        p.category(p.imcoords(:,1)>center & p.imcoords(:,2)>center) = 4;
        p.category(p.imcoords(:,1)<center & p.imcoords(:,2)>center) = 3;
        p.category(p.imcoords(:,1)<center & p.imcoords(:,2)<center) = 2;
        p.category(p.imcoords(:,1)>center & p.imcoords(:,2)<center) = 1;
        p.proto_order = [4,3,2,1];
    end
...
%             end
            ...
            if data.Response(tt)~=0
%                 % outline their answer too
%                 Screen('FrameRect', w, p.fixColor, p.protoFramePos_leftright(data.Response(tt),:),p.rectWidthPix);
...
%             % outline the correct answer
%             Screen('FrameRect', w, p.rectColor, p.protoFramePos_leftright(p.category(tt),:),p.rectWidthPix);
