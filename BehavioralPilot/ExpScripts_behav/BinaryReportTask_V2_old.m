...
% parameters
    ...
    % 56 total images presented
    
    % total time = 2s blank start + 56*(1+1+1+2) = 282 seconds
    % 282/60 = 4.7 minutes
...
try
    ...
    if p.Training
        p.expName = sprintf('ShapeBinaryTaskV2_bound%d_TRAINING',p.which_bound);
    else
        p.expName = sprintf('ShapeBinaryTaskV2_bound%d',p.which_bound);
    end
...
%     
    ...
    %define binary side labels for the categories
    p.side = zeros(size(p.imlist));
    p.side(p.imcoords(:,p.which_bound)>center) = 1;
    p.side(p.imcoords(:,p.which_bound)<center) = 2;
...
%     p.fixColor = [0,0,0];
    ...
    proto_orders = [1,2;2,1];
    
    % for each possible order, define some screen positions. We'll select a
    % random set of positions each time we want to draw the prototypes.
    for oo=1:size(proto_orders,1)
        
        my_order = proto_orders(oo,:);
        protocenters_pix = protocenters_pix(my_order,:);
            
        % this is the text that labels each prototype image
        p.boundTextPos{oo} = [protocenters_pix(:,1), (protocenters_pix(:,2)+(p.protoStimHeightPix/2 + p.protoStimHeightPix/5))];
            
        % define where to draw the images themselves
        p.protoFramePos{oo} = [ protocenters_pix(:,1)-p.protoStimWidthPix/2, ...
...
%     end
    ...
    % choose a random spatial order to show the prototype images
    oo = datasample(1:2,1);
%     my_order = proto_orders(oo,:);
    
    % draw the quadrant prototype images
    for bb=1:2
        Screen('DrawTexture', w, proto_ims(bb).imtext,[],p.protoFramePos{oo}(bb,:));        
...
%     Screen('BlendFunction', w, GL_DST_ALPHA, GL_ONE_MINUS_DST_ALPHA);
        ...
        if p.Training
           ...
           % draw the quadrant prototype images
            ...
            if data.Response(tt)~=0
                ...
                Screen('FrameRect', w, p.fixColor, p.protoFramePos{oo}(data.Response(tt),:),p.rectWidthPix);
             
                if data.Response(tt)==p.side(tt)
                    showtext = 'Correct!\n\n';
                else
            ...
            % outline the correct answer
            Screen('FrameRect', w, p.rectColor, p.protoFramePos{oo}(p.side(tt),:),p.rectWidthPix);
                          
            DrawFormattedText(w, showtext, 'center', p.centerPix(2)-p.protoStimHeightPix*1.25, p.white);
            Screen('DrawDots', w, [0,0], p.fixSizePix, p.fixColor, p.centerPix, 0); 
            Screen('DrawingFinished', w);
            Screen('Flip', w);
    ...
    acc = mean(data.Response(inds2use)==p.side(inds2use));
