...
% [nParts x nSess]
task_list_even = [1,2;2,1;3,3]; % which category boundary?
map_list_even = [1,2;1,2;1,2];   % which response mapping?
position_list_even = [1,2;1,2;1,2];  % which positions will we show prototypes on the screen?
% even num subject, start with second sess (otherwise start with
% the first session)
task_list_odd = task_list_even(:,[2,1]);
map_list_odd = map_list_even(:,[2,1]);
position_list_odd = position_list_even(:,[2,1]);
...
for si = 1:nSubj
    ...
    % sessions
    alldat = dir(fullfile(root, 'Data_V5',sprintf('%s_MainTaskMRI_nofeedback*.mat',subinit)));
    names = {alldat.name};
    ...
    if nSess==1
       nSess=2;
       sess_dates = [sess_dates, sess_dates];
%         error('%s: has %d sessions\n',subinit,nSess)
    ...
    for se = 1:nSess
        ...
        for xx = 1:nPartsTotal
            ...
            filename = fullfile(root, 'Data_V5',sprintf('%s_MainTaskMRI_nofeedback_sess%d_part%d_%s.mat',subinit,se,xx,sess_dates{se}));
            load(filename);
            p = TheData(1).p;
...
for se = 1:2
    ...
    legend(lax,legendlabs,'Location','EastOutside')
...
%             plot(distances_from_bound(~not_sampled),vals(~not_sampled),'-','Color',[0.8, 0.8, 0.8])
        ...
        if nSubj>1
            ...
            meanvals = props;
            sevals = zeros(size(props));
        end
        
        not_sampled = isnan(meanvals);
        errorbar(distances_from_bound(~not_sampled), meanvals(~not_sampled),sevals(~not_sampled),'Color','k');
