...
for si = 1:nSubj
    ...
    % sessions
    alldat = dir(fullfile(root, 'Data_V3',sprintf('%s_ShapeTask_*.mat',subinit)));
    names = {alldat.name};
    ...
    for se = 1:nSess
        ...
        for xx = 1:nPartsTotal
            ...
            filename = fullfile(root, 'Data_V3',sprintf('%s_ShapeTask_sess%d_part%d_%s.mat',subinit,se,xx,sess_dates{se}));
            load(filename);
            p = TheData(1).p;
...
%             end
            ...
            for rr=1:nRunsPerPart
                ...
                % because of a bug in the code, these are actually the
                % images that were presented...but they are not the ones
                % that feedback was based on! 
                actual_category = p.category(p.imlist);
                actual_quadrant = p.quadrant(p.imlist);
                actual_points = p.points(p.imlist,:);
...
%                 correct_resp = p.category(~timeout)';
                correct_resp = actual_category(~timeout)';
...
%                 cat_groups = [p.proto_order(1:2);p.proto_order(3:4)];
                ...
                % them the same again.
%                 if curr_map==2
%                     category_unmapped = 3-actual_category;
%                 else
%                     category_unmapped = actual_category;
%                 end
                ...
                if curr_task<3
                    ...
                    for tr = 1:nTrials
                        ...
                        % boundary (difficulty)?
                        sign = double(actual_points(tr,curr_task)>center);
                        sign(sign==0) = -1;
...
%                         nansum(nRespFail(si,curr_task,:))+nansum(nRespRight(si,curr_task,:))+nansum(nRespLeft(si,curr_task,:))
                ...
                for tr =1:nTrials
                    ind1 = find(all_pts==actual_points(tr,1));
                    ind2 = find(all_pts==actual_points(tr,2));
