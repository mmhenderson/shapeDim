...
for si = 1:nSubj
    ...
    % sessions
    alldat = dir(fullfile(root, 'Data_V5',sprintf('%s_MainTaskMRI_feedback*.mat',subinit)));
    names = {alldat.name};
...
%         error('%s: has %d sessions\n',subinit,nSess)
    ...
    for se = 1:nSess
        ...
        for xx = 1:nPartsTotal
            ...
            filename = fullfile(root, 'Data_V5',sprintf('%s_MainTaskMRI_feedback_sess%d_part%d_%s.mat',subinit,se,xx,sess_dates{se}));
            load(filename);
            p = TheData(1).p;
