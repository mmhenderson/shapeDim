...
savepath = fullfile(root, 'Analysis','Decoding_results','Decode_responsemapping_all.mat');
...
%             fingerlabs = mainSig(vv).ResponseLabels(inds2use);
            ...
            un = unique(maplabs);
            num_each = sum(repmat(maplabs,1,numel(un))==repmat(un',numel(maplabs),1),1);
%             assert(all(num_each==num_each(1)))

%             cvlabs = ceil(mainSig(vv).RunLabels(inds2use)/2);
...
%         maplabs = nan(size(repSig(vv).IsRepeatLabels(inds2use)));
        maplabs = repSig(vv).MapLabels;
%         isreplabs = repSig(vv).IsRepeatLabels(inds2use);
%         
%         maplabs(maplabs==2) = 1+isreplabs(maplabs==2);
%         maplabs(maplabs==1) = 2-isreplabs(maplabs==1);
%         
        un = unique(maplabs);
        num_each = sum(repmat(maplabs,1,numel(un))==repmat(un',numel(maplabs),1),1);
        assert(all(num_each==num_each(1)))
