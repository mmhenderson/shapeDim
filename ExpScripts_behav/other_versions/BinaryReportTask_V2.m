% BINARY REPORT TASK - SHAPE SPACE 
% Subject sees a sequence of images - must report the category of each
% image, according to a binary linear division.
% There are two different binary division schemes that can be used. 
...
try
    ...
    p.imagedir=fullfile(root, 'Stimuli/AmpGrid3_full_grey/');
    ...
    bound = input('Boundary? (1 or 2)  --> ','s'); 
    if isempty(bound)  
        error('enter an integer between 1 and 2')
    end
    p.which_bound = str2double(bound);
    if mod(p.which_bound,1) || p.which_bound<1 || p.which_bound>2
        error('enter an integer between 1 and 2')
    end
    
    mapping = input('Mapping? (1 or 2)  --> ','s'); 
    if isempty(mapping)  
        error('enter an integer between 1 and 2')
    end
    p.which_mapping = str2double(mapping);
    if mod(p.which_mapping,1) || p.which_mapping<1 || p.which_mapping>2
        error('enter an integer between 1 and 2')
    end
    ...
    if p.Training
        p.expName = sprintf('ShapeBinaryTaskV2_bound%d_mapping%d_TRAINING',p.which_bound,p.which_mapping);
    else
        p.expName = sprintf('ShapeBinaryTaskV2_bound%d_mapping%d',p.which_bound,p.which_mapping);
    end
...
%     p.nTrials = 8;
   ...
   % first I'm defining all possible images that we can use in this task.
    ...
    if p.which_bound==1
        proto_coords = [proto_pts(1),center; proto_pts(2),center];
    else
        proto_coords = [center, proto_pts(1); center, proto_pts(2)];
    end
%     proto_coords = [proto_pts(2), proto_pts(2); proto_pts(1), proto_pts(2); proto_pts(1), proto_pts(1); proto_pts(2), proto_pts(1)];
    ...
    % are ambiguous 
    bound_inds = find(all_grid_points(:,p.which_bound)==center);
...
%     
    ...
    %define binary side labels for the categories - use the response
    %mapping now to define which shape is which "side"
    p.category = zeros(size(p.imlist));
    if p.which_mapping==1
        % p.category tells us which finger the subject should press on each
        % trial
        p.category(p.imcoords(:,p.which_bound)<center) = 1;
        p.category(p.imcoords(:,p.which_bound)>center) = 2;
        p.proto_order = [1, 2];
    else
        p.category(p.imcoords(:,p.which_bound)>center) = 1;
        p.category(p.imcoords(:,p.which_bound)<center) = 2;
...
%     p.fixColor = [0,0,0];
    ...
    protocenters_pix = repmat(p.centerPix,2, 1) + p.protoDistPix*[-1,0; 1,0];
    
    % define where to draw the images themselves
    % first listing the frame positions in left-right order
    p.protoFramePos_leftright = [ protocenters_pix(:,1)-p.protoStimWidthPix/2, ...
                        protocenters_pix(:,2)-p.protoStimHeightPix/2,...
                        protocenters_pix(:,1)+p.protoStimWidthPix/2,...
                        protocenters_pix(:,2)+p.protoStimHeightPix/2];
    %outlinepos is where to draw the colored outlines (during feedback), a
    %bit smaller than the full image square so they won't overlap.
    p.protoOutlinePos_leftright = [ protocenters_pix(:,1)-p.outlineProp*p.protoStimWidthPix/2, ...
                        protocenters_pix(:,2)-p.outlineProp*p.protoStimHeightPix/2,...
                        protocenters_pix(:,1)+p.outlineProp*p.protoStimWidthPix/2,...
                        protocenters_pix(:,2)+p.outlineProp*p.protoStimHeightPix/2];
    % now listing the same positions, but re-ordered according to which
    % image should go in which position
    protocenters_pix = protocenters_pix(p.proto_order,:);
    p.protoFramePos = p.protoFramePos_leftright(p.proto_order,:);      
...
%                 DrawFormattedText(w, sprintf('%d',bb), p.boundTextPos{oo}(bb,1)+p.textAdjPix(1),p.boundTextPos{oo}(bb,2)+p.textAdjPix(2),p.white);
            ...
            if data.Response(tt)~=0
                ...
                Screen('FrameRect', w, p.fixColor, p.protoOutlinePos_leftright(data.Response(tt),:),p.rectWidthPix);
            ...
            % outline the correct answer
            Screen('FrameRect', w, p.rectColor, p.protoOutlinePos_leftright(p.category(tt),:),p.rectWidthPix);
