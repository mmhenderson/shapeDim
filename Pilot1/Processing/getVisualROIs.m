...
VisualMask = true;
if exist([experiment_path, 'VOIs/S', char(subnum)], 'dir')
    if ~isempty(dir([experiment_path, 'VOIs/S', char(subnum), '/*h_*nii.gz'])) % if there are visual masks found
    ...
    mkdir([experiment_path, 'VOIs/S', char(subnum)]);
end
...
%     delete([experiment_path, 'VOIs/S' char(subnum) '/*h_*nii.gz']); %delete retino content from VOI folder 
    ...
    s = Label2VOI(FS_sbj, ROIs, hemis, [func_path, 'MCTemplateXFM01.nii.gz'],...
        [func_path, 'Func2Anat_auto.dat'], [experiment_path, 'VOIs/S' char(subnum) '/']);
    if s == 1
