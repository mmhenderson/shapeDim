...
% Following all this processing - the common space we end up aligned to is
% the anatomical template found in doreti folder, with a transformation
% applied by regheadercenterMOD.mat. All files with the suffix _REG_MC_DET
% will be aligned to this space (which is also specified in the file
% MCTemplateXFM01.nii.gz)
% Note MCTemplateXFM02.nii.gz may be slightly different, this turns out to
% be ok once motion correction is applied.
...
if WannaCoregister
    for session = preproc_sessions
        ...
        % bboutput is in the exact same space as the anatomy from Doreti. 
        % after this transformation below, we get MCTemplateXFM, which is
        % this same space plus the transformation specified by
        % regheadercenterMOD.mat
...
if WannaMC    
    ...
    for session = preproc_sessions 
        ...
        if ~isempty(dir([subpath, sprintf('%02d',session), '_REG_MC*.nii.gz']))
            rsp = input(['Motion corrected volumes from session ', num2str(session),' already exist, Overwrite? (y/n) --> '],'s');
            if strcmp(rsp,'n')
                DoMoco = false;
            end
        end
