...
roi_dir = fullfile(exp_path,'VOIs_MD',subnum);
if ~isfolder(roi_dir)
    mkdir(roi_dir);
end
...
%% load a teplate file
template_file = fullfile(subretfolder,'mri','orig.nii.gz');
template_nii = load_nifti(template_file);

% get out some important fields from it
template_sform = template_nii.sform;
template_vol = template_nii.vol;
...
% converting to new matrix indices
...
mask_new = zeros(template_dims);
mask_new(mask_inds) =1;

% make and save the new nifti
mask_nii = template_nii;
mask_nii.vol = mask_new;
tal_brain_mask_nii = fullfile(exp_path,'DataPreproc', subnum, 'MNI_BRAIN_MASK_test.nii.gz');
...
% make and save the new nifti
mask_xfm_nii = template_nii;
mask_xfm_nii.vol = mask_xfm;
tal_brain_mask_xfm = fullfile(exp_path,'DataPreproc', subnum, 'MNI_BRAIN_MASK_REG2ANAT.nii.gz');
...
for ll=1:length(labels)
    ...
    % make and save the single-label nifti
    roi_single_xfm = zeros(size(rois_vol));
    roi_single_xfm(roi_inds_xfm) = 1;
    
    roi_xfm_nii = template_nii;
    roi_xfm_nii.vol = roi_single_xfm;
    roi_xfm_file = fullfile(exp_path,'VOIs_MD',subnum,sprintf('MDROI_label%d_REG2ANAT.nii.gz',ll));
    save_nifti(roi_xfm_nii, roi_xfm_file);
    
end

% make and save the (full) nifti
md_xfm_nii = template_nii;
md_xfm_nii.vol = rois_all_xfm;
md_xfm_file = fullfile(exp_path,'DataPreproc', subnum, 'MDROI_REG2ANAT.nii.gz');
