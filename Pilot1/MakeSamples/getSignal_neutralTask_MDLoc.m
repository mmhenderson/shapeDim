...
for ss=1:length(sublist)
    ...
    load([root, 'SampleFile_MDLoc_S', my_subject,'.mat'],'samplesMain','main','ROIs','all_vox_concat');

    fn2save = [root 'NeutralTaskSignalByTrial_MDLoc_S' my_subject, '.mat'];
   
    %% vis areas to use
    my_areas = {};
    for aa = 1:size(ROIs,2)
        if isempty(ROIs(1,aa).name)
            underscores = find(ROIs(2,aa).name=='_');
            my_areas{aa} = ROIs(2,aa).name(underscores(1)+1:underscores(2)-1);
        else
            underscores = find(ROIs(1,aa).name=='_');
            my_areas{aa} = ROIs(1,aa).name(underscores(1)+1:underscores(2)-1);
        end
    end
