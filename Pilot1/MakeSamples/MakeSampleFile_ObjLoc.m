...
filename = fullfile(out_path, ['SampleFile_ObjLoc_S', subnum]);
...
for hh = 1:length(hemis)   
    ...
    % end.
%     not_retino = find(contains({VOIfiles.name}, 'loc'));
    ob_loc = contains({VOIfiles.name},'objcatloc');
    ...
    for file_idx = 1:length(VOIfiles) 
        ...
        if hh==1
            ...
            v_idx = find(contains({ROIs(1,:).name},ROI_name(4:end)));
            if isempty(v_idx)
               v_idx = size(ROIs,2)+1;
